    ...
    for dagger_iter in range(dagger_iterations):
        ...
        for _ in range(rollout_episodes):
            ...
            while not done and steps < max_steps:
                ...
                state_tensor_bc = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
                expert_action_tensor = policy_bc(state_tensor_bc)
                expert_action = expert_action_tensor.detach().squeeze(0).numpy() 
